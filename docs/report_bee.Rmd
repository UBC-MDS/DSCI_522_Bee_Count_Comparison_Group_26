...
The histograms and the violin plots show that the data is skewed to the right because most counts are zero. Even if the response variable was continous (e.g. weight instead of count), using means would not be appropriate since the data is skewed.
...
>
> 1.  Independence: The observations must be independent of one another. <br>
> 2.  Mean=Variance: By definition, the mean of a Poisson random variable must be equal to its variance. <br>
> 3.  Linearity: The log of the mean rate, $log(\\lambda_i$), must be a linear function of $x$.</font>
